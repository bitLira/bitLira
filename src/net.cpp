...
static const char *strDNSSeed[][2] = {
    {"Frozenbox bitLira zero node", "node0.bitlira.frozenbox.org"},
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BitLira is probably already running."), addrBind.ToString().c_str());
        else
